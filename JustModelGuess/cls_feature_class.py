...
class FeatureClass:
    ...
    def get_labels_for_file(self, _desc_file):
        ...
        return label_mat
          ...
          # ADD THIS NEW FUNCTION TO YOUR cls_feature_class.py FILE

    # REPLACE the old get_features_for_file function with this new one

    def get_features_for_file(self, audio_filename):
        """
        Processes a single audio file from a full path and returns the combined feature matrix.
        """
        # 1. Load the audio directly using the provided full path
        audio_in, fs = self._load_audio(audio_filename)

        # 2. Get the linear spectrogram
        spect = self._spectrogram(audio_in)

        # 3. Get the mel spectrogram from the linear spectrogram
        mel_spect = self._get_mel_spectrogram(spect)

        # 4. Get the spatial features and combine them
        feat = None
    ...
    def extract_all_labels(self):
        ...
        for file_cnt, file_name in enumerate(os.listdir(self._desc_dir)):
            try:  # ---- START OF THE ADDED CODE ----
            # This is the original code, now inside the 'try' block
                wav_filename = '{}.wav'.format(file_name.split('.')[0])
            ...
            # This code will run ONLY if the ValueError happens
                print("\n" + "="*50)
                print(f"ERROR: Found the corrupted file that is causing the crash!")
                print(f"--> The problem file is: {file_name}")
                print("="*50 + "\n")
            # We stop the script here so you can fix the file.
                break  # ---- END OF THE ADDED CODE ----
